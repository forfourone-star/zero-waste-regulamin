...
* Adres e-mail (Google Sign-In): Do autoryzacji konta i obsługi portfela Kredytów.
* Zdjęcia z aparatu/galerii: Przesyłane w czasie rzeczywistym do analizy AI (skanowanie paragonów/lodówki) – nie są przez nas trwale przechowywane.
* Tokeny powiadomień Push: Do komunikacji systemowej i rodzinnej.
* Advertising ID: W celu wyświetlania reklam AdMob (wyłącznie za Twoją zgodą).

## 3. Czego NIE zbieramy?
* NIE śledzimy Twojej lokalizacji (GPS).
* NIE pobieramy Twojej listy kontaktów.

## 4. Prywatność lokalna
Aliasy i nazwy własne członków rodziny są zapisywane wyłącznie lokalnie w pamięci Twojego urządzenia (AsyncStorage) i nie są przesyłane do naszej bazy danych.

## 5. Komu udostępniamy dane?
Twoje dane mogą być przetwarzane przez zaufanych partnerów technicznych:
* Supabase (AWS): Przechowywanie bazy danych i list zakupów.
* OpenRouter / Google (Gemini) / Black Forest Labs (Flux): Analiza danych i generowanie treści AI.
* RevenueCat: Obsługa procesów zakupowych.
